...
class CustomPromptAgent(BaseNewsletterAgent):
    ...
    async def _process_prompt(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not custom_prompt.strip():
                return {"success": False, "error": "No custom prompt provided"}

            # Analyze the prompt
            analysis = await self._analyze_prompt_content(
                custom_prompt, user_preferences
            )
            ...
            research_params = await self._generate_research_parameters(
                enhanced_prompt, analysis
            )

            # Generate writing guidelines
            writing_guidelines = await self._generate_writing_guidelines(
                enhanced_prompt, analysis
            )

            # Store the processed prompt for future reference
            if user_id:
                await self._store_prompt_processing(
                    user_id,
                    {
    ...
    async def _enhance_prompt(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "original_prompt": custom_prompt,
                "enhanced_prompt": enhanced_prompt,
                "enhancements_applied": self._get_enhancements_applied(
    ...
    async def _execute_full_prompt_processing(
    ...
    async def _analyze_prompt_content(
        self, custom_prompt: str, user_preferences: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        detected_tone = user_preferences.get(
            "tone", "professional"
    ...
    async def _enhance_prompt_with_context(
        ...
        missing_topics = [
            topic for topic in user_topics if topic not in detected_topics
        ]
        if missing_topics and len(detected_topics) < 3:
            enhanced_parts.append(
                f"Also include content related to: {', '.join(missing_topics[:2])}"
            )

        # Add tone preference if not specified
        if analysis.get("detected_tone") == user_preferences.get(
            "tone", "professional"
        ):
            ...
            enhanced_parts.append(
                f"Use a {preferred_tone} tone throughout the newsletter."
            )
        ...
        if reading_patterns.get("preferred_topics"):
            ...
            if not any(topic in detected_topics for topic in preferred):
                enhanced_parts.append(
                    f"Consider including content about {', '.join(preferred)} based on reading history."
                )
    ...
    async def _generate_research_parameters(
        ...
        days_back_mapping = {"today": 1, "this_week": 7, "recent": 3, "trending": 1}
        ...
        return {
            ...
            "custom_query": enhanced_prompt
            if search_strategy == "custom_query"
            else None,
            "days_back": days_back,
    ...
    async def _store_prompt_processing(
        self, user_id: str, processing_data: Dict[str, Any]
    ) -> bool:
