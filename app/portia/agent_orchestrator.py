...
class NewsletterAgentOrchestrator:
    ...
    async def generate_newsletter(
        ...
        try:
            ...
            if custom_prompt:
                ...
                if prompt_result["success"]:
                    ...
                    research_context.update({
                        "custom_prompt": custom_prompt,
                        "topics": research_params.get("topics", research_context["topics"]),
            ...
            if not articles:
                workflow_results["error"] = "No articles found for newsletter generation"
                return workflow_results
            ...
            if custom_prompt and workflow_results["steps"].get("custom_prompt", {}).get("success"):
                writing_guidelines = workflow_results["steps"]["custom_prompt"].get("writing_guidelines", {})
            ...
            if send_email and user_email:
                ...
                subject_line = subject_lines[0] if subject_lines else f"📧 {newsletter.get('title', 'Your Newsletter')}"
            ...
            stored = await self.memory_service.store_newsletter_history(
                user_id=user_id,
                newsletter_data=newsletter_with_id
            )
    ...
    async def research_content_only(
        ...
        if custom_prompt:
            ...
            return await self.research_agent.execute_task("search_custom_prompt", context)
        else:
            return await self.research_agent.execute_task("search_by_topics", context)
    
    async def get_newsletter_history(self, user_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Get user's newsletter history"""
        return await self.memory_service.get_newsletter_history(user_id, limit)
    
    async def get_user_engagement_metrics(self, user_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def process_custom_prompt_only(
        ...
        if not user_preferences:
            ...
            user_preferences = prefs_result.get("preferences", {}) if prefs_result["success"] else {}
    ...
    async def test_agent_integrations(self) -> Dict[str, Any]:
        ...
        test_results["email_service"] = {
            "status": "active" if self.email_service.api_key else "inactive",
            "message": "Resend API key configured" if self.email_service.api_key else "Resend API key not configured"
        }
        
        # Test memory service
        test_results["memory_service"] = {
            "status": "active" if self.memory_service.redis_url else "inactive",
            "message": "Upstash Redis configured" if self.memory_service.redis_url else "Upstash Redis not configured"
        }
