    ...
    try:
        ...
        for user in users:
            ...
            if user.preferences:
                ...
                logger.info(
                    f"      Preferences: {prefs.topics}, tone: {prefs.tone}, frequency: {prefs.frequency}"
                )
            ...
            for newsletter in newsletters:
                logger.info(
                    f"         Newsletter: {newsletter.title} (Status: {newsletter.status.value})"
                )
                logger.info(
                    f"         Type: {newsletter.newsletter_type.value}, Word count: {newsletter.word_count}"
                )
                logger.info(
                    f"         Topics: {newsletter.topics_covered}, Tone: {newsletter.tone_used}"
                )
                ...
                for history in history_entries:
                    logger.info(
                        f"            History: {history.delivery_status.value} at {history.sent_at}"
                    )
        ...
        tech_users = (
            ...
            .filter(text("user_preferences.topics::jsonb ? 'tech'"))
            .all()
        )
        ...
        opened_history = (
            ...
            .filter(NewsletterHistory.delivery_status == DeliveryStatus.OPENED)
            .all()
        )
...
def test_model_validation():
    ...
    try:
        ...
        if sample_user and sample_user.preferences:
            ...
            logger.info(
                f"   Excluded sources: {prefs.excluded_sources} (type: {type(prefs.excluded_sources)})"
            )
        ...
        if sample_newsletter:
            ...
            logger.info(
                f"   Content sections: {sample_newsletter.content_sections} (type: {type(sample_newsletter.content_sections)})"
            )
            logger.info(
                f"   Topics covered: {sample_newsletter.topics_covered} (type: {type(sample_newsletter.topics_covered)})"
            )
            logger.info(
                f"   Sources used: {sample_newsletter.sources_used} (type: {type(sample_newsletter.sources_used)})"
            )
