...
class RAGSystem:
    ...
    async def embed_and_store_newsletter(
        self, newsletter_id: str, user_id: str, newsletter_data: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            metadata = {
                ...
                "user_id": user_id,
                "type": "newsletter",
                "generated_at": newsletter_data.get(
                    "generated_at", datetime.utcnow().isoformat()
                ),
                "article_count": newsletter_data.get("metadata", {}).get(
                    "article_count", 0
                ),
                "tone": newsletter_data.get("metadata", {})
                .get("user_preferences", {})
                .get("tone", "professional"),
                "topics": json.dumps(
                    newsletter_data.get("metadata", {})
                    .get("user_preferences", {})
                    .get("topics", [])
                ),
                "custom_prompt": newsletter_data.get("metadata", {}).get(
                    "custom_prompt", ""
                ),
                    ...
                    similar_newsletters.append(
                        {
                            ...
                            "content_preview": match.metadata.get(
                                "content_preview", ""
                            ),
    ...
    async def get_content_recommendations(
        ...
        try:
            ...
            article_titles = [
                article.get("title", "") for article in current_articles[:3]
            ]
            ...
            similar_newsletters = await self.retrieve_similar_newsletters(
                user_id=user_id, query=query, top_k=10, similarity_threshold=0.6
            )
            ...
            if tone_counts:
                ...
                recommendations["tone_recommendations"] = {
                    ...
                    "confidence": tone_counts[preferred_tone]
                    / len(similar_newsletters),
                ...
                recommendations["topic_suggestions"] = [
                    {
                        "topic": topic,
                        "frequency": freq,
                        "relevance": freq / len(similar_newsletters),
                    }
            ...
            recommendations["personalization_level"] = (
                "high" if len(similar_newsletters) >= 3 else "medium"
            )
    ...
    async def analyze_user_preferences(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for newsletter in all_newsletters:
                tone = newsletter.get("tone", "professional")
                analysis["preferred_tones"][tone] = (
                    analysis["preferred_tones"].get(tone, 0) + 1
                )
            ...
            for newsletter in all_newsletters:
                topics = newsletter.get("topics", [])
                for topic in topics:
                    analysis["topic_interests"][topic] = (
                        analysis["topic_interests"].get(topic, 0) + 1
                    )
            ...
            if article_counts:
                analysis["content_patterns"] = {
                    "avg_articles_per_newsletter": sum(article_counts)
                    / len(article_counts),
                    "preferred_length": "short"
                    if sum(article_counts) / len(article_counts) < 5
                    else "long",
                    "consistency_score": 1.0
                    - (max(article_counts) - min(article_counts))
                    / max(article_counts, 1),
                }

            # Generate insights
            if analysis["preferred_tones"]:
                most_used_tone = max(
                    analysis["preferred_tones"], key=analysis["preferred_tones"].get
                )
                analysis["engagement_insights"].append(
                    f"Prefers {most_used_tone} tone in newsletters"
                )

            if analysis["topic_interests"]:
                top_topics = sorted(
                    analysis["topic_interests"].items(),
                    key=lambda x: x[1],
                    reverse=True,
                )[:3]
                analysis["engagement_insights"].append(
                    f"Most interested in: {', '.join([t[0] for t in top_topics])}"
                )
    ...
    async def generate_personalized_content_context(
        ...
        try:
            ...
            recommendations = await self.get_content_recommendations(
                user_id=user_id, current_topics=topics, current_articles=articles
            )

            context["recommendations"] = recommendations
            context["rag_available"] = (
                recommendations.get("personalization_level") != "basic"
            )
            ...
            context["content_strategy"] = {
                "recommended_tone": recommendations.get("tone_recommendations", {}).get(
                    "preferred_tone", user_preferences.get("tone", "professional")
                ),
                "optimal_article_count": user_analysis.get("content_patterns", {}).get(
                    "avg_articles_per_newsletter", 5
                ),
                "focus_topics": [
                    t["topic"] for t in recommendations.get("topic_suggestions", [])[:3]
                ],
            }
