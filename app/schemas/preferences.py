...
class PreferencesBase(BaseModel):
    topics: List[str] = Field(default_factory=list, description="List of topics user is interested in")
    tone: str = Field(default="professional", description="Writing tone preference")
    frequency: str = Field(default="weekly", description="Newsletter delivery frequency")
    custom_instructions: Optional[str] = Field(default=None, description="Custom instructions for content generation")
    preferred_length: str = Field(default="medium", description="Preferred article length")
...
class PreferenceRecommendation(BaseModel):
    ...
    suggested_topics: Optional[List[str]] = Field(default=None, description="Suggested topics to add")
    suggested_changes: Optional[List[str]] = Field(default=None, description="Suggested changes")
