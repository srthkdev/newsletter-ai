...
@router.put("/{user_id}")
async def update_preferences(user_id: str, preferences_data: Dict[str, Any], db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not preferences:
            ...
            preferences.frequency = preferences_data.get("frequency", preferences.frequency)
            preferences.custom_instructions = preferences_data.get("custom_instructions", preferences.custom_instructions)
            preferences.max_articles_per_newsletter = preferences_data.get("max_articles", preferences.max_articles_per_newsletter)
            preferences.preferred_length = preferences_data.get("preferred_length", preferences.preferred_length)
            preferences.preferred_send_time = preferences_data.get("send_time", preferences.preferred_send_time)
            preferences.timezone = preferences_data.get("timezone", preferences.timezone)
            preferences.include_trending = preferences_data.get("include_trending", preferences.include_trending)
...
async def analyze_preferences(user_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error analyzing preferences: {str(e)}")

@router.post("/topics")
async def update_topics(topics: List[str], user_id: str = "demo_user", db: Session = Depends(get_db)):
...
@router.post("/tone")
async def update_tone(tone: str, user_id: str = "demo_user", db: Session = Depends(get_db)):
