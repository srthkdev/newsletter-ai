...
async def signup(request: OTPRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not email_sent:
            ...
            print(
                f"🔐 OTP for {email}: {otp_code} (expires in {OTP_EXPIRY_MINUTES} minutes)"
            )
...
async def verify_otp(request: OTPVerification, db: Session = Depends(get_db)):
    ...
    try:
        ...
        await cache_service.set(
            f"session:{session_token}", session_data, ttl=SESSION_EXPIRY_HOURS * 3600
        )
...
async def logout(session_token: str = None):
    ...
    try:
        ...
        return AuthResponse(
            success=True, message="Logged out successfully", redirect_url="/"
        )
