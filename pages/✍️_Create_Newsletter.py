...
# Page configuration
st.set_page_config(
    page_title="Create Newsletter - Newsletter AI",
    page_icon="✍️",
    layout="wide"
)
...
def generate_custom_newsletter(prompt: str, preferences: Optional[Dict[str, Any]] = None) -> tuple[bool, str, Optional[Dict[str, Any]]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return False, error_data.get("detail", "Failed to generate newsletter"), None
            
    except requests.exceptions.ConnectionError:
        return False, "Cannot connect to server. Please make sure the API is running.", None
    except requests.exceptions.Timeout:
        return False, "Request timed out. Newsletter generation is taking longer than expected.", None
...
def send_newsletter(newsletter_id: str) -> tuple[bool, str]:
    ...
    try:
        response = requests.post(
            f"{API_BASE_URL}/newsletters/{newsletter_id}/send",
            timeout=30
        )
...
def main():
    ...
    # Main content tabs
    tab1, tab2, tab3 = st.tabs(["✍️ Create Prompt", "📋 Examples", "🎯 Advanced Options"])
...
def show_prompt_creation(preferences: Optional[Dict[str, Any]]):
    ...
    if preferences and not preferences.get('is_default', False):
        ...
        st.warning("⚠️ You haven't set up preferences yet. Your custom prompt will use default settings.")
...
- Any special focus areas""",
    ...
    if custom_prompt:
        ...
        has_tone = any(tone in custom_prompt.lower() for tone in ['professional', 'casual', 'technical', 'formal', 'friendly'])
        has_topics = any(topic in custom_prompt.lower() for topic in ['ai', 'tech', 'business', 'startup', 'science', 'health'])
        has_specifics = any(word in custom_prompt.lower() for word in ['focus', 'include', 'cover', 'about', 'regarding'])
...
def show_example_prompts():
    ...
    st.markdown("Click on any example to use it as a starting point for your custom newsletter")
    ...
    with col1:
        topic_focus = st.selectbox(
            "Main topic focus",
            ["", "Technology", "Business", "Science", "Health", "Finance", "Marketing", "Innovation"],
...
def show_advanced_options(preferences: Optional[Dict[str, Any]]):
    ...
    st.session_state.advanced_options = {
        ...
        "override_length": override_length if override_length != "Use default" else None,
        "override_max_articles": override_max_articles,
        "include_trending_override": include_trending_override if include_trending_override != "Use default" else None,
        ...
        "priority_sources": [s.strip() for s in priority_sources.split(",") if s.strip()] if priority_sources else []
    }
...
def show_generation_section():
    ...
    with col1:
        if st.button("🔍 Preview Newsletter", type="secondary", use_container_width=True):
            with st.spinner("Generating preview..."):
                ...
                if advanced_options.get('override_length'):
                    combined_preferences['preferred_length'] = advanced_options['override_length']
                if advanced_options.get('override_max_articles'):
                    combined_preferences['max_articles'] = advanced_options['override_max_articles']
        ...
        if st.button("📧 Generate & Send", type="primary", use_container_width=True):
            ...
            if success and newsletter_data:
                ...
                if newsletter_id:
                    ...
                    if send_success:
                        st.success(f"✅ Newsletter generated and sent! Check your email.")
                        st.balloons()
                    else:
                        st.error(f"❌ Newsletter generated but sending failed: {send_message}")
                else:
